...
def run_single_simulation(num_AP, sim_time, num_channels, scan_period, selection_period, busy_factor, busy_factor_self, idle_factor, neighbor_matrix, attack_time, release_time):
    ...
    # Initialize attack and release times
    attack_times = np.zeros((num_AP, num_channels))
    release_times = np.zeros((num_AP, num_channels))
    ...
    for time in range(sim_time):
        for AP in range(num_AP):
            ...
            if (time + start_sel[AP]) % selection_period == 0:
                ch = channels[AP]
                busy = np.sum(np.mean((channels[:, np.newaxis] == ch) * traffic[:, max(0, time - selection_period):time] * neighbor_matrix[AP, :, np.newaxis], axis=1))
                ...
                for c in range(num_channels):
                    attack_times[AP, c] += (g_channel_busy[AP, c] - attack_times[AP, c]) / attack_time
                    release_times[AP, c] += (g_channel_busy[AP, c] - release_times[AP, c]) / release_time

                # Calculate the score as the maximum of attack and release times
                scores = np.maximum(attack_times[AP, :], release_times[AP, :])

                min_avg = np.min(scores)
                best_ch = np.argmin(scores) + 1
...
def simulate_channel_switches(num_AP, selection_period, sim_time=12*60*60, num_channels=4, scan_period=5, busy_factor=0.8, busy_factor_self=0.5, idle_factor=0.99, num_simulations=30, density=0.5, attack_time=10, release_time=10):
    neighbor_matrix = generate_neighbor_matrix(num_AP, density)
    with mp.Pool(mp.cpu_count()) as pool:
        results = pool.starmap(run_single_simulation, [(num_AP, sim_time, num_channels, scan_period, selection_period, busy_factor, busy_factor_self, idle_factor, neighbor_matrix, attack_time, release_time) for _ in range(num_simulations)])
...
attack_time = 10  # Define attack time here
release_time = 10  # Define release time here
...
for sp in selection_periods:
    for num_AP in num_APs:
        ...
        np.random.seed(42)
        avg_switches, std_switch, total_distance = simulate_channel_switches(num_AP, sp, sim_time=sim_time, density=density, attack_time=attack_time, release_time=release_time)
