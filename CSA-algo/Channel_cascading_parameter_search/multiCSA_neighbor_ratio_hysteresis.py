...
def run_single_simulation(num_AP, sim_time, num_channels, scan_period, selection_period, busy_factor, busy_factor_self, idle_factor, neighbor_matrix, upper_limit, lower_limit):
    ...
    for time in range(sim_time):
        for AP in range(num_AP):
            ...
            if (time + start_sel[AP]) % selection_period == 0:
                ...
                if g_cur_channel_busy[AP] > upper_limit * min_avg and g_cur_channel_busy[AP] > lower_limit:
                    # Move channel
                    channels[AP] = best_ch
...
def simulate_channel_switches(num_AP, selection_period, sim_time=12*60*60, num_channels=4, scan_period=5, busy_factor=0.8, busy_factor_self=0.5, idle_factor=0.99, num_simulations=30, density=0.5, upper_limit=1.25, lower_limit=0.15):
    neighbor_matrix = generate_neighbor_matrix(num_AP, density)
    with mp.Pool(mp.cpu_count()) as pool:
        results = pool.starmap(run_single_simulation, [(num_AP, sim_time, num_channels, scan_period, selection_period, busy_factor, busy_factor_self, idle_factor, neighbor_matrix, upper_limit, lower_limit) for _ in range(num_simulations)])
...
upper_limit = 1.25  # Define upper limit here
lower_limit = 0.15  # Define lower limit here
...
for sp in selection_periods:
    for num_AP in num_APs:
        ...
        np.random.seed(42)
        avg_switches, std_switch, total_distance = simulate_channel_switches(num_AP, sp, sim_time=sim_time, density=density, upper_limit=upper_limit, lower_limit=lower_limit)
